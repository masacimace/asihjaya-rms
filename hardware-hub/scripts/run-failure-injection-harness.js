...
function createJob(seed, kind, apiUrl) {
  ...
  if (kind === "label") {
    ...
    payload = {
      schemaVersion: 1,
      templateId: "jewelry_barbell_inter_v3",
      templateVersion: 3,
      printerProfileId: "sato_cg408_jewelry_barbell_inter_v3",
      itemId: uuid(`item:${seed}`),
      copies: 1,
      fields: {
        sku: `SKU-${seed}`,
        barcode: crypto.createHash("sha256").update(String(seed)).digest("hex").slice(0, 12).toUpperCase(),
        masterProductName: "Cincin Emas Simulasi",
        itemDisplayName: "Cincin Emas Simulasi Item",
        weightGram: "2.35",
        purityPercent: "75",
...
function createRuntime({ root, cloud, agentId, scenario, apiUrl, outputDir, delayMs = 20 }) {
  ...
  const adapterFactory = createHardwareAdapterFactory({
    ...
    satoTemplateId: "jewelry_barbell_inter_v3",
    satoPrinterProfileId: "sato_cg408_jewelry_barbell_inter_v3",
    satoCopies: 1,
