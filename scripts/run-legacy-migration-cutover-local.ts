...
function resolveCommand(command: "npm" | "docker", args: string[]) {
  ...
  if (!npmExecPath) {
    throw new Error(
      "npm_execpath tidak tersedia. Jalankan runner melalui npm run test:migration:cutover:local.",
    );
  }
...
function waitForPostgres() {
  ...
  throw new Error("PostgreSQL 17 cutover-test belum siap setelah 60 detik.");
}

console.log("Menyalakan PostgreSQL 17 disposable untuk cutover integration test...");
run(
  "docker",
...
try {
  ...
  console.log("Menjalankan transactional cutover integration suite...");
  run("npm", ["run", "test:migration:cutover"]);
} finally {
  console.log("Menghapus PostgreSQL cutover-test beserta volume sementara...");
  run(
    "docker",
    ["compose", "-f", composeFile, "down", "--volumes", "--remove-orphans"],
    { allowFailure: true },
