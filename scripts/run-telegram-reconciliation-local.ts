...
const projectName = `asihjaya-rms-telegram-reconciliation-${process.pid}`;
...
function findAvailablePort(): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      assert(address && typeof address === "object", "Port test Telegram reconciliation tidak tersedia.");
...
function resolveNpm(args: string[]) {
  ...
  if (!npmExecPath) {
    throw new Error("npm_execpath tidak tersedia. Jalankan melalui npm run test:telegram-reconciliation:local.");
  }
...
async function waitForPostgres(composeArgs: string[], environment: NodeJS.ProcessEnv, databaseUrl: string) {
  ...
  while (Date.now() < deadline) {
    ...
    const host = internal ? await canConnect(databaseUrl) : false;
    if (host) {
      consecutive += 1;
  ...
  throw new Error("PostgreSQL 17 Telegram reconciliation belum stabil setelah 60 detik.");
}
...
console.log("Menyalakan PostgreSQL 17 disposable untuk Telegram 2C.10 reconciliation...");
...
try {
  ...
  console.log("[reconciliation] Menjalankan migration sampai schema Telegram terbaru...");
  runNpm(["run", "db:migrate"], environment);
  console.log("[reconciliation] Menjalankan database reconciliation + idempotency checks...");
  runNpm(["run", "check:telegram-operations", "--", "--database"], environment);
  console.log(
    "Telegram 2C.10 local reconciliation rehearsal passed: missed weekly/monthly periods, setting cutoff, idempotency, dan immutable finance snapshot source.",
  );
} finally {
  console.log("Menghapus PostgreSQL Telegram 2C.10 reconciliation beserta volume sementara...");
  run("docker", [...composeArgs, "down", "--volumes", "--remove-orphans"], environment, true);
}
