...
const [schema, action, listPage, detailPage, migration] = await Promise.all([
  ...
  readFile(
    "drizzle/0016_p1c1_manual_payment_reconciliation.sql",
    "utf8",
  ),
]);
...
assert.match(migration, /WHEN "settlement_status" = 'matched' THEN 'pending_settlement'/);
assert.match(migration, /WHEN "settlement_status" = 'settled' THEN 'reconciled'/);
assert.match(migration, /payments\.reconciliation\.view/);
