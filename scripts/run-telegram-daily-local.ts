...
const projectName = `asihjaya-rms-telegram-daily-${process.pid}`;
...
function findAvailablePort(): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      assert(address && typeof address === "object", "Port test Telegram daily finance tidak tersedia.");
...
function resolveNpm(args: string[]) {
  ...
  if (!npmExecPath) {
    throw new Error("npm_execpath tidak tersedia. Jalankan melalui npm run test:telegram-daily:local.");
  }
...
async function waitForPostgres(
  ...
  throw new Error(
    "PostgreSQL 17 Telegram daily finance test belum stabil melalui published port setelah 60 detik.",
  );
}
...
console.log("Menyalakan PostgreSQL 17 disposable untuk Telegram 2C.5 daily finance...");
...
try {
  ...
  console.log("[daily] Menjalankan migration sampai schema Telegram terbaru...");
  runNpm(["run", "db:migrate"], environment);

  console.log("[daily] Menjalankan integration checks finance snapshot + daily outbox...");
  runNpm(["run", "check:telegram-daily", "--", "--database"], environment);

  console.log(
    "Telegram 2C.5 local daily finance rehearsal passed: cost snapshot, finance snapshot, Dana Titip, payment breakdown, idempotency, dan closing daily outbox.",
  );
} finally {
  console.log("Menghapus PostgreSQL Telegram 2C.5 beserta volume sementara...");
  run("docker", [...composeArgs, "down", "--volumes", "--remove-orphans"], environment, true);
}
