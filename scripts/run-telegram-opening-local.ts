...
const projectName = `asihjaya-rms-telegram-opening-${process.pid}`;
...
function findAvailablePort(): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      assert(address && typeof address === "object", "Port test Telegram opening tidak tersedia.");
...
function resolveNpm(args: string[]) {
  ...
  if (!npmExecPath) {
    throw new Error("npm_execpath tidak tersedia. Jalankan melalui npm run test:telegram-opening:local.");
  }
...
async function waitForPostgres(
  ...
  throw new Error(
    "PostgreSQL 17 Telegram opening test belum stabil melalui published port setelah 60 detik.",
  );
}
...
console.log("Menyalakan PostgreSQL 17 disposable untuk Telegram 2C.4 opening...");
...
try {
  ...
  console.log("[opening] Menjalankan migration sampai schema Telegram terbaru...");
  runNpm(["run", "db:migrate"], environment);

  console.log("[opening] Menjalankan integration checks opening + outbox...");
  runNpm(["run", "check:telegram-opening", "--", "--database"], environment);

  console.log(
    "Telegram 2C.4 local opening rehearsal passed: business date, setting guard, destination guard, idempotency, immutable payload, dan transaction rollback.",
  );
} finally {
  console.log("Menghapus PostgreSQL Telegram 2C.4 beserta volume sementara...");
  run("docker", [...composeArgs, "down", "--volumes", "--remove-orphans"], environment, true);
}
