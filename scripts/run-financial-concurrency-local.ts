...
function resolveCommand(command: "npm" | "docker", args: string[]) {
  ...
  if (!npmExecPath) {
    throw new Error(
      "npm_execpath tidak tersedia. Jalankan runner melalui npm run test:financial:local.",
    );
  }
...
function waitForPostgres() {
  ...
  throw new Error("PostgreSQL 17 financial-test belum siap setelah 60 detik.");
}

console.log("Menyalakan PostgreSQL 17 disposable untuk financial/concurrency test...");
...
try {
  ...
  console.log("Menjalankan financial & concurrency integration suite...");
  run("npm", ["run", "test:financial"]);
} finally {
  console.log("Menghapus PostgreSQL financial-test beserta volume sementara...");
  run("docker", ["compose", "-f", composeFile, "down", "--volumes", "--remove-orphans"], {
    allowFailure: true,
  });
}
