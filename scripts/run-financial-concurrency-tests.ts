...
process.env.SESSION_SECRET ??= "test-session-secret-000000000000000000000000000000000000000";
process.env.RECEIPT_VERIFICATION_SECRET ??= "test-receipt-secret-00000000000000000000000000000000000000";
process.env.CUSTOMER_HISTORY_SESSION_SECRET ??= "test-history-session-secret-000000000000000000000000000000000";
process.env.CUSTOMER_HISTORY_PIN_PEPPER ??= "test-history-pin-pepper-00000000000000000000000000000000000";
process.env.SECURITY_RATE_LIMIT_SECRET ??= "test-rate-limit-secret-000000000000000000000000000000000000";
process.env.PDF_RENDER_TOKEN_SECRET ??= "test-pdf-render-secret-000000000000000000000000000000000000";
process.env.HARDWARE_AGENT_CREDENTIAL_ENCRYPTION_KEY ??=
  "test-agent-encryption-key-000000000000000000000000000000000";

const { runFinancialConcurrencySuite } = await import(
  "../tests/integration/financial-concurrency-suite"
);

await runFinancialConcurrencySuite();
