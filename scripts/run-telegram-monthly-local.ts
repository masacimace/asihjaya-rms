...
const projectName = `asihjaya-rms-telegram-monthly-${process.pid}`;
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      assert(
        ...
        "Port test Telegram monthly finance tidak tersedia.",
      );
...
function resolveNpm(args: string[]) {
  ...
  if (!npmExecPath) {
    throw new Error(
      "npm_execpath tidak tersedia. Jalankan melalui npm run test:telegram-monthly:local.",
    );
  }
...
async function waitForPostgres(
  ...
  throw new Error(
    "PostgreSQL 17 Telegram monthly finance test belum stabil melalui published port setelah 60 detik.",
  );
}
...
console.log("Menyalakan PostgreSQL 17 disposable untuk Telegram 2C.8 monthly finance...");
...
try {
  ...
  console.log("[monthly] Menjalankan migration sampai schema Telegram terbaru...");
  runNpm(["run", "db:migrate"], environment);

  console.log("[monthly] Menjalankan integration checks monthly aggregation + outbox...");
  runNpm(["run", "check:telegram-monthly", "--", "--database"], environment);

  console.log(
    "Telegram 2C.8 local monthly rehearsal passed: calendar month, leap-year boundaries, immutable snapshot aggregation, delayed generation, cost completeness, previous-month comparison, Dana Titip, dan idempotency.",
  );
} finally {
  console.log("Menghapus PostgreSQL Telegram 2C.8 beserta volume sementara...");
  run("docker", [...composeArgs, "down", "--volumes", "--remove-orphans"], environment, true);
}
