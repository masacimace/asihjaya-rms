...
function getAbsolutePath(key: string) {
  ...
  if (!normalized) throw new Error("Kunci bukti pembayaran tidak valid.");
  ...
  if (!absolutePath.startsWith(`${root}${path.sep}`)) {
    throw new Error("Lokasi bukti pembayaran tidak valid.");
  }

  return absolutePath;
}

export function paymentEvidenceKeyBelongsToOrganization(
  key: string,
...
export function getPaymentEvidenceUrl(key: string | null) {
  if (!key || !normalizeEvidenceKey(key)) return null;
  return `/media/payment-evidence/${key
    .split("/")
    .map((segment) => encodeURIComponent(segment))
    .join("/")}`;
}

export async function storePaymentEvidenceFile({
  file,
  organizationId,
}: {
  file: File;
  ...
  const key = `organizations/${organizationId}/payment-evidence/${randomUUID()}.webp`;
...
export async function deletePaymentEvidenceFile(key: string) {
  const normalized = normalizeEvidenceKey(key);
  if (!normalized) throw new Error("Kunci bukti pembayaran tidak valid.");
...
export async function readPaymentEvidenceFile(key: string) {
  const normalized = normalizeEvidenceKey(key);
  if (!normalized) throw new Error("Kunci bukti pembayaran tidak valid.");
  ...
  if (getStorageDriver() === "s3") {
    ...
    if (!response.Body) throw new Error("Bukti pembayaran tidak ditemukan.");
