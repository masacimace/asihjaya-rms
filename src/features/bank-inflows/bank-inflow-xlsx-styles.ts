...
function styleSummaryWorksheet(source: string) {
  ...
  styled = styleCells(styled, 1, 1, [0, 1, 2, 3, 4, 5], STYLE.title);
  styled = styleCells(styled, 2, 8, [0], STYLE.metadataLabel);
  styled = styleCells(styled, 2, 8, [1], STYLE.metadataValue);

  for (const row of [10, 16]) {
    styled = styleCells(styled, row, row, [0, 1, 2, 3, 4, 5], STYLE.sectionHeader);
  }

  styled = styleCells(styled, 11, 12, [0], STYLE.moneyLabel);
  styled = styleCells(styled, 11, 12, [1], STYLE.moneyValue);
  styled = setCellStyle(styled, "A13", STYLE.grandLabel);
  styled = setCellStyle(styled, "B13", STYLE.grandMoney);
  styled = setCellStyle(styled, "A14", STYLE.metricLabel);
  styled = setCellStyle(styled, "B14", STYLE.metricValue);
  ...
  for (let row = bankDataStartRow; row <= lastRow; row += 1) {
    const alternate = (row - bankDataStartRow) % 2 === 1;
    styled = setCellStyle(
      styled,
      `A${row}`,
      alternate ? STYLE.tableTextAlt : STYLE.tableText,
    );
  ...
  styled = setRowHeight(styled, 10, 23);
  for (let row = 11; row <= 14; row += 1) styled = setRowHeight(styled, row, 21);
  styled = setRowHeight(styled, 15, 8);
  styled = setRowHeight(styled, 16, 23);
  styled = setRowHeight(styled, 17, 28);
  for (let row = 18; row <= lastRow; row += 1) styled = setRowHeight(styled, row, 22);

  return styled;
}

function styleMovementWorksheet(source: string) {
  ...
  for (let row = 7; row <= lastRow; row += 1) {
    ...
    styled = styleCells(styled, row, row, [0, 1, 2, 3, 4, 5, 11], textStyle);
    styled = styleCells(styled, row, row, [6, 7], centerStyle);
    styled = styleCells(styled, row, row, [8, 9, 10], moneyStyle);
  }
...
export function styleBankInflowWorkbookBuffer(buffer: Buffer) {
  ...
  const transforms = new Map<string, (source: string) => string>([
    ...
    ["xl/worksheets/sheet2.xml", styleMovementWorksheet],
  ]);
