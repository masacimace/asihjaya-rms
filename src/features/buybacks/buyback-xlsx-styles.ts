...
function styleSummaryWorksheet(source: string) {
  ...
  const payoutSectionRow = 28;
  const payoutHeaderRow = 29;
  const payoutDataStartRow = 30;
  const payoutDataEndRow = getLastWorksheetRow(styled);

  styled = styleCells(styled, 1, 1, [0, 1, 2], STYLE.title);
  ...
  for (const row of [10, 21]) {
    styled = styleCells(styled, row, row, [0, 1, 2], STYLE.sectionHeader);
  }
  styled = styleCells(
    styled,
    payoutSectionRow,
  ...
  for (let row = payoutDataStartRow; row <= payoutDataEndRow; row += 1) {
    const alternate = (row - payoutDataStartRow) % 2 === 1;
    styled = setCellStyle(
      styled,
      `A${row}`,
    ...
    styled = setCellStyle(
      styled,
      `B${row}`,
      alternate ? STYLE.tableNumberAlt : STYLE.tableNumber,
    );
    styled = setCellStyle(
      styled,
      `C${row}`,
  ...
  for (const row of [10, 21, payoutSectionRow]) styled = setRowHeight(styled, row, 23);
  ...
  styled = setRowHeight(styled, payoutHeaderRow, 26);
  for (let row = payoutDataStartRow; row <= payoutDataEndRow; row += 1) {
...
function styleTransactionWorksheet(source: string) {
  ...
  for (let row = 7; row <= lastRow; row += 1) {
    ...
    styled = styleCells(
      ...
      row,
      row,
      [0, 1, 2, 3, 4, 5, 6, 7, 11, 16, 17],
      textStyle,
    );
    styled = styleCells(styled, row, row, [8], numberStyle);
    styled = styleCells(styled, row, row, [9, 10], centerStyle);
    styled = styleCells(styled, row, row, [12, 13, 14, 15], moneyStyle);
  }
...
function styleItemWorksheet(source: string) {
  ...
  for (let row = 2; row <= lastRow; row += 1) {
    ...
    styled = styleCells(styled, row, row, [0, 1, 3, 6, 7, 8, 9, 12], textStyle);
    styled = styleCells(styled, row, row, [2, 5, 14, 15], centerStyle);
    styled = styleCells(styled, row, row, [4, 10, 11], numberStyle);
    styled = styleCells(styled, row, row, [13], moneyStyle);
  }
...
export function styleBuybackWorkbookBuffer(buffer: Buffer) {
