...
const IMAGE_PATH_PATTERN = /^images\/(masters|physical)\/([^/]+)$/;
const ALLOWED_DIRECTORY_PATHS = new Set([
  "images/",
  "images/masters/",
  "images/physical/",
]);
...
function getExtension(fileName: string): string {
  ...
  return dotIndex < 0 ? "" : fileName.slice(dotIndex).toLocaleLowerCase("en-US");
}
...
function assertSafeImageFileName(fileName: string, archivePath: string): void {
  ...
  if (!normalized || normalized !== fileName || normalized.startsWith(".")) {
    throw archiveError("ARCHIVE_IMAGE_NAME_INVALID", `Nama image tidak valid: ${archivePath}.`);
  }
  if (!ALLOWED_IMAGE_EXTENSIONS.has(getExtension(normalized))) {
    throw archiveError("ARCHIVE_FILE_UNSUPPORTED", `File tidak didukung pada archive: ${archivePath}.`);
  }
}

export function inspectProductBatchArchive(buffer: Buffer): ProductBatchArchiveInspection {
  let zip;
  try {
  ...
  for (const entry of zip.entries) {
    if (entry.isDirectory) {
      if (!ALLOWED_DIRECTORY_PATHS.has(entry.path)) {
        throw archiveError("ARCHIVE_PATH_UNSUPPORTED", `Folder tidak didukung: ${entry.path}.`);
      }
      continue;
    }

    if (entry.path === "products.xlsx") {
      if (workbookEntry) {
        throw archiveError("ARCHIVE_WORKBOOK_DUPLICATE", "Archive hanya boleh mempunyai satu products.xlsx.");
      }
      if (entry.uncompressedSize > PRODUCT_BATCH_IMPORT_LIMITS.workbookBytes) {
        throw archiveError("ARCHIVE_WORKBOOK_TOO_LARGE", "products.xlsx melebihi batas 5 MB.");
      }
      workbookEntry = entry;
      continue;
    }
    ...
    if (!imageMatch) {
      throw archiveError("ARCHIVE_PATH_UNSUPPORTED", `File/path tidak didukung: ${entry.path}.`);
    }
    ...
    if (entry.uncompressedSize === 0) {
      throw archiveError("ARCHIVE_IMAGE_EMPTY", `Image kosong ditolak: ${entry.path}.`);
    }
    if (entry.uncompressedSize > PRODUCT_BATCH_IMPORT_LIMITS.imageBytes) {
      throw archiveError("ARCHIVE_IMAGE_TOO_LARGE", `Image melebihi batas 5 MB: ${entry.path}.`);
    }
  ...
  if (!workbookEntry) {
    throw archiveError("ARCHIVE_WORKBOOK_MISSING", "products.xlsx wajib tersedia tepat di root ZIP.");
  }
  ...
  return {
    ...
    totalUncompressedBytes: zip.entries.reduce((total, entry) => total + entry.uncompressedSize, 0),
  };
}

export function extractProductBatchArchiveEntry(buffer: Buffer, entry: StrictZipEntry): Buffer {
  try {
