      ...
      {issues.map((issue, index) => (
        ...
        >
          <div className="flex flex-wrap items-center gap-2 font-semibold">
            ...
            {issue.field ? (
              <span className="font-mono opacity-75">{issue.field}</span>
            ) : null}
          </div>
          <p className="mt-1">{issue.message}</p>
          {issue.archivePath ? (
            <p className="mt-1 break-all opacity-75">{issue.archivePath}</p>
          ) : null}
        </div>
      ))}
    </div>
...
function ItemCompact({
  ...
  const effectiveMedia =
    row.media ??
    (row.effectiveImageSource === "master" ? row.masterMedia : null);
  return (
    <div className="min-w-0 rounded-2xl border border-neutral-200 bg-white p-4">
      <div className="flex min-w-0 items-start gap-3">
        <MediaThumbnail
          ...
          label={asText(row.normalizedPayload.row_key)}
        />
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="text-xs text-[var(--muted)]">
                PHYSICAL_PRODUCTS row {row.rowNumber}
              </p>
          ...
          <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2 xl:grid-cols-4">
            <div>
              <dt className="text-[var(--muted)]">Master</dt>
              <dd className="break-words font-medium">
                {asText(row.normalizedPayload.master_key)}
              </dd>
            </div>
            <div>
              <dt className="text-[var(--muted)]">Outlet</dt>
              <dd className="break-words font-medium">
                {row.resolvedOutletCode ??
                  asText(row.normalizedPayload.outlet_code)}
              </dd>
            </div>
            <div>
              <dt className="text-[var(--muted)]">Condition</dt>
              <dd className="font-medium">
                {asText(row.normalizedPayload.condition)}
              </dd>
            </div>
            <div>
              <dt className="text-[var(--muted)]">Lokasi</dt>
              <dd className="break-words font-medium">
                {asText(row.normalizedPayload.location_code)}
              </dd>
            </div>
          </dl>
          ...
          {row.validationErrors.length || row.validationWarnings.length ? (
            <details className="mt-3">
              <summary className="cursor-pointer text-xs font-semibold text-neutral-700">
                Lihat {row.validationErrors.length} error ·{" "}
                {row.validationWarnings.length} warning
              </summary>
...
export default async function ProductBatchImportPreviewPage({
  ...
  const [{ sessionId }, queryParams] = await Promise.all([
    params,
    searchParams,
  ]);
  ...
  const filteredMasters = preview.masters.filter(
    (row) =>
      rowMatchesStatus(row.validationStatus, statusFilter) &&
      masterMatchesSearch(row, normalizedSearch),
  );
  const filteredItems = preview.items.filter(
    (row) =>
      rowMatchesStatus(row.validationStatus, statusFilter) &&
      itemMatchesSearch(row, normalizedSearch),
  );
  ...
  const allIssues = [
    ...preview.masters.flatMap((row) => [
      ...row.validationErrors.map((issue) => ({
        ...
        key: asText(row.normalizedPayload.master_key),
        issue,
      })),
    ...
    ...preview.items.flatMap((row) => [
      ...row.validationErrors.map((issue) => ({
        kind: "Item",
        rowNumber: row.rowNumber,
        key: asText(row.normalizedPayload.row_key),
        issue,
      })),
      ...row.validationWarnings.map((issue) => ({
        kind: "Item",
    ...
    .filter(
      (entry) =>
        !normalizedSearch ||
        `${entry.kind} ${entry.key} ${entry.issue.code} ${entry.issue.field ?? ""} ${entry.issue.message}`
          .toLocaleLowerCase("id-ID")
  ...
  const summaryCards: Array<{
    ...
    {
      label: "Master",
      value: preview.session.totalMasterRows,
      icon: Package,
      color: "text-neutral-900",
    },
    {
      label: "Item fisik",
      value: preview.session.totalItemRows,
      icon: Store,
      color: "text-neutral-900",
    },
    {
      label: "Master valid",
      value: preview.session.validMasterRows,
      icon: CheckCircle2,
      color: "text-emerald-700",
    },
    {
      label: "Invalid rows",
      value: preview.session.invalidRows,
      icon: XCircle,
      color: preview.session.invalidRows ? "text-red-700" : "text-emerald-700",
    },
    {
      label: "Warnings",
      value: preview.session.warningCount,
      icon: AlertTriangle,
      color: preview.session.warningCount
        ? "text-amber-700"
        : "text-neutral-700",
    },
  ];
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl min-w-0 flex-col gap-6 overflow-x-clip pb-8">
          ...
          <dl className="grid gap-2 rounded-2xl bg-neutral-50 p-4 text-xs">
            <div className="flex justify-between gap-3">
              ...
              <dd className="break-all text-right font-mono">
                {preview.session.id.slice(0, 8)}…
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-[var(--muted)]">Operator</dt>
              <dd className="text-right font-medium">
                {preview.session.createdByName}
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-[var(--muted)]">Divalidasi</dt>
              <dd className="text-right font-medium">
                {formatDateTime(
                  preview.session.validatedAt,
                  auth.organization.timezone,
                )}
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-[var(--muted)]">Expired</dt>
              <dd className="text-right font-medium">
                {formatDateTime(
                  preview.session.expiresAt,
                  auth.organization.timezone,
                )}
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-[var(--muted)]">Ukuran ZIP</dt>
              <dd className="font-medium">
                {formatBytes(preview.session.fileSizeBytes)}
              </dd>
            </div>
          </dl>
        </div>
            ...
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1">
                  ...
                  {formatNumber(result.items.length)} Product Item sudah menjadi
                  data bisnis nyata.
                </p>
                <p className="mt-1 text-xs text-emerald-800">
                  Committed{" "}
                  {formatDateTime(
                    result.session.committedAt,
                    auth.organization.timezone,
                  )}{" "}
      ...
      <section className="rounded-3xl border border-[var(--border)] bg-white p-4 sm:p-5">
        <div className="flex min-w-0 flex-wrap gap-2 border-b border-neutral-100 pb-4">
          {(
            [
              ...
              [
                "images",
                `Images (${preview.media.filter((row) => row.status !== "deleted").length})`,
              ],
        ...
        <form
          className="mt-4 grid min-w-0 gap-3 sm:grid-cols-[minmax(0,1fr)_180px_auto]"
          method="get"
        >
          ...
          <label className="relative min-w-0">
            ...
            <input
              name="q"
              defaultValue={search}
              placeholder="Cari master_key, row_key, nama, outlet..."
              className="h-10 w-full min-w-0 rounded-xl border border-neutral-200 bg-white pl-9 pr-3 text-sm outline-none focus:border-neutral-400"
            />
          </label>
          <select
            name="status"
            defaultValue={statusFilter}
            className="h-10 min-w-0 rounded-xl border border-neutral-200 bg-white px-3 text-sm"
          >
      ...
      <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-start">
        <main className="min-w-0 space-y-4">
            ...
            filteredMasters.length ? (
              filteredMasters.map((row) => {
                ...
                return (
                  <details
                    key={row.id}
                    open={row.validationStatus === "invalid"}
                    className="group min-w-0 rounded-3xl border border-[var(--border)] bg-white p-4 sm:p-5"
                  >
                    <summary className="flex min-w-0 cursor-pointer list-none items-start gap-3 [&::-webkit-details-marker]:hidden">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          <div className="min-w-0">
                            <p className="text-xs text-[var(--muted)]">
                              PRODUCT_MASTERS row {row.rowNumber}
                            </p>
                            <h2 className="mt-1 break-words font-semibold text-neutral-950">
                              {asText(row.normalizedPayload.name)}
                            </h2>
                            <p className="mt-1 break-words text-sm text-[var(--muted)]">
                              {asText(row.normalizedPayload.master_key)} ·{" "}
                              {row.resolvedCategoryCode ??
                                asText(row.normalizedPayload.category_code)}
                            </p>
                          </div>
                          <div className="flex items-center gap-2">
                            <StatusBadge status={row.validationStatus} />
                            <ChevronRight className="size-4 text-neutral-400 transition group-open:rotate-90" />
                          </div>
                        </div>
                        <div className="mt-3 flex flex-wrap gap-2 text-xs">
                          ...
                          <span className="rounded-lg bg-neutral-100 px-2.5 py-1">
                            status {asText(row.normalizedPayload.status)}
                          </span>
                          {row.validationErrors.length ? (
                            <span className="rounded-lg bg-red-50 px-2.5 py-1 font-semibold text-red-700">
                              {row.validationErrors.length} error
                            </span>
                          ) : null}
                    ...
                    <div className="mt-5 border-t border-neutral-100 pt-5">
                      <dl className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
                        <div>
                          ...
                          <dd className="mt-1 break-words font-medium">
                            {asText(row.normalizedPayload.brand)}
                          </dd>
                        </div>
                        <div>
                          ...
                          <dd className="mt-1 break-words font-medium">
                            {asText(row.normalizedPayload.material)}
                          </dd>
                        </div>
                        <div>
                          <dt className="text-xs text-[var(--muted)]">
                          ...
                          <dd className="mt-1 break-words font-medium">
                            {asText(row.normalizedPayload.collection)}
                          </dd>
                        </div>
                        <div>
                          <dt className="text-xs text-[var(--muted)]">Image</dt>
                          <dd className="mt-1 break-all font-medium">
                            {asText(row.normalizedPayload.primary_image)}
                          </dd>
                        </div>
                      </dl>
                      {row.validationErrors.length ||
                      ...
                      <div className="mt-5 space-y-3">
                        ...
                        {children.length ? (
                          ...
                          <p className="rounded-xl bg-neutral-50 p-4 text-sm text-[var(--muted)]">
                            Tidak ada item yang terhubung ke master staging ini.
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <p className="rounded-3xl border border-dashed border-neutral-300 bg-white p-8 text-center text-sm text-[var(--muted)]">
                Tidak ada Product Master yang cocok dengan filter.
              </p>
            )
          ) : null}
          ...
          {view === "items" ? (
            filteredItems.length ? (
              filteredItems.map((row) => (
                <ItemCompact key={row.id} sessionId={sessionId} row={row} />
              ))
            ) : (
              <p className="rounded-3xl border border-dashed border-neutral-300 bg-white p-8 text-center text-sm text-[var(--muted)]">
                Tidak ada Physical Product yang cocok dengan filter.
              </p>
            )
          ) : null}
                ...
                .map((media) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-[var(--muted)]">
                      {media.entityKind} · {formatBytes(media.byteSize)} ·{" "}
          ...
          {view === "issues" ? (
            allIssues.length ? (
              allIssues.map((entry, index) => (
                <article
                  key={`${entry.kind}-${entry.rowNumber}-${entry.issue.code}-${index}`}
                  className={cn(
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <p className="text-xs text-[var(--muted)]">
                        {entry.kind} · row {entry.rowNumber}
                      </p>
                  ...
                  <div className="mt-3">
                    <IssueList issues={[entry.issue]} />
                  </div>
                </article>
              ))
            ) : (
              <p className="rounded-3xl border border-dashed border-neutral-300 bg-white p-8 text-center text-sm text-[var(--muted)]">
                Tidak ada error/warning yang cocok dengan filter.
              </p>
            )
          ) : null}
        </main>
        ...
        <aside className="min-w-0 space-y-4 xl:sticky xl:top-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-[var(--muted)]">
                  Session ready dapat di-commit secara atomic setelah konfirmasi
                  final.
                </p>
              </div>
            </div>
          ...
          <section className="rounded-3xl border border-[var(--border)] bg-white p-5">
            ...
            <p className="mt-1 text-xs leading-5 text-[var(--muted)]">
              Workbook berisi summary, master errors, item errors, dan warnings
              dengan formula-injection protection.
            </p>
            <Link
              href={`/admin/produk/import/${sessionId}/errors`}
              className="mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-neutral-200 bg-white px-3 text-sm font-semibold text-neutral-800 hover:bg-neutral-50"
            >
              <Download className="size-4" />
            ...
            {preview.session.status === "completed" ? (
              <Link
                href={`/admin/produk/import/${sessionId}/result`}
                className="mt-2 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-neutral-950 px-3 text-sm font-semibold !text-white hover:bg-neutral-800"
              >
                <Download className="size-4" />
          ...
          <section className="rounded-3xl border border-[var(--border)] bg-white p-5 text-xs leading-5">
            ...
            <dl className="mt-3 space-y-2">
              <div>
                <dt className="text-[var(--muted)]">SHA-256 ZIP</dt>
                <dd className="mt-1 break-all font-mono text-[11px]">
                  {preview.session.fileSha256}
                </dd>
              </div>
              <div>
                <dt className="text-[var(--muted)]">Template</dt>
                <dd className="font-medium">
                  v{preview.session.templateVersion}
                </dd>
              </div>
              {preview.session.failureCode ? (
