    ...
    >
      <div className="relative flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className={cn("text-sm font-medium", tone === "dark" ? "text-neutral-300" : "text-[var(--muted)]")}>
            {title}
          </p>
          <p className="mt-2 break-words text-2xl font-semibold tracking-tight">{value}</p>
          <p className={cn("mt-2 text-xs leading-5", tone === "dark" ? "text-neutral-400" : "text-[var(--muted)]")}>
            {helper}
          </p>
        </div>
        <span
...
function MovementBadge({ row }: { row: BankInflowMovement }) {
  ...
  return (
    ...
    >
      {isReceipt ? <ArrowUpRight className="size-3.5" /> : <ArrowDownRight className="size-3.5" />}
    ...
    <div className="space-y-6 pb-10">
        ...
        <div className="grid gap-6 p-6 lg:grid-cols-[1fr_22rem] lg:items-start lg:p-7">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--muted)]">
              Rekonsiliasi penerimaan EDC dan Transfer Bank berdasarkan payment ledger, termasuk refund yang sudah dikonfirmasi.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-neutral-50 p-4">
            ...
            <p className={cn(
              ...
              data.summary.netAmount < 0 ? "text-red-700" : "text-neutral-950",
            )}>
              {formatMoney(data.summary.netAmount)}
            </p>
            <p className="mt-1 text-sm leading-6 text-[var(--muted)]">
              {formatInteger(data.summary.movementCount)} mutasi · {data.period.label}
            </p>
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-xs leading-5 text-[var(--muted)]">
                Periode: {data.period.label} · Outlet: {currentOutlet ? `${currentOutlet.code} — ${currentOutlet.name}` : "Semua outlet"}
              </p>
            </div>
          </div>
            ...
            <label className="space-y-1.5 text-sm font-medium text-neutral-700">
              ...
              >
                ...
                {data.outlets.map((outlet) => (
                  <option key={outlet.id} value={outlet.id}>{outlet.code} — {outlet.name}</option>
                ))}
              </select>
            </label>
      ...
      <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-white">
        <div className="border-b border-[var(--border)] px-5 py-4">
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--muted)]">
            EDC dan Transfer menunjukkan penerimaan kotor. Refund dikurangkan untuk mendapatkan nilai bersih per bank.
          </p>
        </div>
            ...
            <table className="min-w-[820px] w-full text-sm">
              <thead className="bg-neutral-50 text-left text-xs uppercase tracking-wide text-neutral-500">
                <tr>
                  ...
                  <th className="px-5 py-3 text-right font-semibold">Transfer</th>
                  <th className="px-5 py-3 text-right font-semibold">Penerimaan</th>
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {data.bankSummary.map((bank) => (
                  <tr key={bank.provider} className="hover:bg-neutral-50/70">
                    <td className="px-5 py-4 font-semibold text-neutral-950">{bank.provider}</td>
                    <td className="px-5 py-4 text-right tabular-nums">{formatMoney(bank.edcAmount)}</td>
                    <td className="px-5 py-4 text-right tabular-nums">{formatMoney(bank.transferAmount)}</td>
                    <td className="px-5 py-4 text-right font-medium tabular-nums text-emerald-700">{formatMoney(bank.receiptAmount)}</td>
                    <td className="px-5 py-4 text-right font-medium tabular-nums text-red-700">{formatMoney(bank.refundAmount)}</td>
                    <td className={cn("px-5 py-4 text-right font-semibold tabular-nums", bank.netAmount < 0 ? "text-red-700" : "text-neutral-950")}>
      ...
      <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-white">
        <div className="flex flex-col gap-2 border-b border-[var(--border)] px-5 py-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="font-semibold text-neutral-950">Detail Mutasi Bank</h2>
            <p className="mt-1 text-xs text-[var(--muted)]">
              Menampilkan {formatInteger(startItem)}–{formatInteger(endItem)} dari {formatInteger(data.total)} mutasi.
            </p>
          </div>
        ...
        {data.rows.length === 0 ? (
          <div className="px-5 py-12 text-center">
            ...
            <p className="mt-3 text-sm font-semibold text-neutral-800">Tidak ada mutasi bank</p>
            <p className="mt-1 text-xs text-[var(--muted)]">Coba ubah periode atau filter laporan.</p>
          </div>
        ) : (
          <>
              ...
              <table className="min-w-[1220px] w-full text-sm">
                <thead className="bg-neutral-50 text-left text-xs uppercase tracking-wide text-neutral-500">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right font-semibold">Penerimaan</th>
                    <th className="px-4 py-3 text-right font-semibold">Refund</th>
                    <th className="px-4 py-3 text-right font-semibold">Bersih</th>
                    <th className="px-4 py-3 font-semibold">Referensi / Profil</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--border)]">
                  {data.rows.map((row) => {
                    const netAmount = row.kind === "receipt" ? row.amount : -row.amount;
                    return (
                      <tr key={row.id} className="hover:bg-neutral-50/70">
                        <td className="whitespace-nowrap px-4 py-4 text-xs text-neutral-600">{formatDateTime(row.occurredAt, auth.organization.timezone)}</td>
                        <td className="px-4 py-4">
                          <Link href={`/admin/penjualan/${row.saleId}`} className="font-semibold text-neutral-950 hover:text-[var(--accent)] hover:underline">
                            {row.invoiceNumber}
                          </Link>
                        </td>
                        <td className="px-4 py-4">
                          <p className="font-medium text-neutral-900">{row.customerName ?? "Walk-in"}</p>
                          <p className="mt-0.5 text-xs text-[var(--muted)]">{row.customerCode ?? row.customerPhone ?? "—"}</p>
                        </td>
                        <td className="px-4 py-4">
                          <p className="font-medium text-neutral-900">{row.outletName}</p>
                          <p className="mt-0.5 text-xs text-[var(--muted)]">{row.outletCode}</p>
                        </td>
                        <td className="px-4 py-4 font-semibold text-neutral-950">{row.provider}</td>
                        <td className="px-4 py-4 text-neutral-700">{row.method === "debit_card" ? "EDC" : "Transfer"}</td>
                        <td className="px-4 py-4"><MovementBadge row={row} /></td>
                        <td className="px-4 py-4 text-right font-medium tabular-nums text-emerald-700">{row.kind === "receipt" ? formatMoney(row.amount) : "—"}</td>
                        <td className="px-4 py-4 text-right font-medium tabular-nums text-red-700">{row.kind === "refund" ? formatMoney(row.amount) : "—"}</td>
                        <td className={cn("px-4 py-4 text-right font-semibold tabular-nums", netAmount < 0 ? "text-red-700" : "text-neutral-950")}>{formatMoney(netAmount)}</td>
                        <td className="max-w-48 truncate px-4 py-4 text-xs text-neutral-600" title={row.providerReference ?? undefined}>{row.providerReference ?? "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
                ...
                return (
                  <article key={row.id} className="space-y-3 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <Link href={`/admin/penjualan/${row.saleId}`} className="truncate font-semibold text-neutral-950 hover:text-[var(--accent)]">
                          {row.invoiceNumber}
                        </Link>
                        <p className="mt-1 text-xs text-[var(--muted)]">{formatDateTime(row.occurredAt, auth.organization.timezone)}</p>
                      </div>
                      <MovementBadge row={row} />
                    </div>
                    <div className="grid grid-cols-2 gap-3 rounded-xl bg-neutral-50 p-3 text-xs">
                      <div><p className="text-[var(--muted)]">Bank / Metode</p><p className="mt-1 font-semibold text-neutral-900">{row.provider} · {row.method === "debit_card" ? "EDC" : "Transfer"}</p></div>
                      <div><p className="text-[var(--muted)]">Bersih</p><p className={cn("mt-1 font-semibold", netAmount < 0 ? "text-red-700" : "text-emerald-700")}>{formatMoney(netAmount)}</p></div>
                      <div><p className="text-[var(--muted)]">Customer</p><p className="mt-1 font-semibold text-neutral-900">{row.customerName ?? "Walk-in"}</p></div>
                      <div><p className="text-[var(--muted)]">Outlet</p><p className="mt-1 font-semibold text-neutral-900">{row.outletName}</p></div>
                    </div>
                    {row.providerReference ? <p className="text-xs text-[var(--muted)]">Ref: {row.providerReference}</p> : null}
                  </article>
                );
              })}
            </div>
        ...
        <div className="flex flex-col gap-3 border-t border-[var(--border)] px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-[var(--muted)]">Halaman {data.page} dari {data.pageCount}</p>
          <div className="flex items-center gap-2">
            {data.page > 1 ? (
              <Link href={buildListUrl(data.page - 1, filters)} className="inline-flex h-9 items-center gap-1 rounded-lg border border-[var(--border)] px-3 text-xs font-semibold text-neutral-700 hover:bg-neutral-50"><ChevronLeft className="size-4" /> Sebelumnya</Link>
            ) : (
              <span className="inline-flex h-9 items-center gap-1 rounded-lg border border-neutral-200 px-3 text-xs font-semibold text-neutral-300"><ChevronLeft className="size-4" /> Sebelumnya</span>
            )}
            {data.page < data.pageCount ? (
              <Link href={buildListUrl(data.page + 1, filters)} className="inline-flex h-9 items-center gap-1 rounded-lg border border-[var(--border)] px-3 text-xs font-semibold text-neutral-700 hover:bg-neutral-50">Berikutnya <ChevronRight className="size-4" /></Link>
            ) : (
              <span className="inline-flex h-9 items-center gap-1 rounded-lg border border-neutral-200 px-3 text-xs font-semibold text-neutral-300">Berikutnya <ChevronRight className="size-4" /></span>
            )}
          </div>
        </div>
      </section>
