...
function collectReceiptMediaKeys(
  documentData: NonNullable<
    Awaited<ReturnType<typeof getBuybackReceiptData>>
  >,
) {
  return Array.from(
    new Set(
...
export async function GET(request: NextRequest, context: RouteContext) {
  const { buybackId } = await context.params;
  ...
  let documentData: Awaited<ReturnType<typeof getBuybackReceiptData>>;
  let rateLimitActor:
    | { type: "user"; id: string }
  ...
  if (hardwareAuth) {
    ...
    documentData = await getBuybackReceiptData({
      buybackId,
  ...
  const filename = sanitizeFilename(
    `${documentData.sale.invoiceNumber}-nota-buyback-${pdf.profile.paper.toLowerCase()}-landscape-${filenameMode}.pdf`,
  );
